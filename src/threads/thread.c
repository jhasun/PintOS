...
/* List processes in THREAD_READY state, that is,
...
/* List of sleeping processes. When TIMER SLEEP is called,
   put current process into this list */
...
/* Initializes the threading system by transforming the code
   ...
   After calling this function, be sure to intialize the page
...
void thread_init(void)
{
  ...
  /* Initialize the system load average as 0. A fixed number 0 is
     numerically equal to an interger 0 */
  load_avg = 0;
...
/* Creates a new kernel thread named NAME with the given intial
   ...
   before thread_create() returns. Contrary, the original thread
   may run for any amount of time before the new thread is
   scheduled. Use a semaphore or some other form of synchronization
...
void thread_set_priority(int new_priority)
{
  ...
  /* If there are threads with higher priority than the current thread,
     call THREAD YIELD */
  try_thread_yield ();
}
...
void thread_set_nice(int nice)
{
  ...
  /* When NICE is changed, the priority is possibly changed */
  thread_update_priority_mlfqs (thread_current ());
...
/* Chooses and returns the next thread to be scheduled. Should
   return a thread from the run queue, unless the run queue is empty.
   If the running thread can continue running, then it will be in the
   run queue. If the run queue is empty, return idle_thread */
static struct thread *next_thread_to_run(void)
{
...
/* Completes a thread switch by activating the new thread's page
   tables, and, if the previous thread is dying, destroy it.

   At this functions invocation, we just switched from threads PREV,
   the new thread is already running, and interrupts are still 
   disabled. This function is normally invoked by thread_schedule()
   as its final action before returning, but the first time a thread
   is scheduled it is called by switch_entry().

   Not safe to call printf() until thread switch is complete. Should
   be added to end of function. */
void thread_schedule_tail(struct thread *prev)
{
  ...
  /* If the thread we switched from is dying, destroy its struyct
     thread. This must happen late so that thread_exit() doesn't
     pull out the rug under itself. Don't free initial_thread
     because because its memory was not obtained via palloc() */
...
/* Schedules a new process. At entry, interrupts must be off
   and the running process's state must have been changed from
...
/* Complete all thread-specific settings while putting the
...
void try_thread_yield (void)
{
  ...
  bool result = !list_empty (&ready_list) &&
		 list_entry (list_back(&ready_list), 
		 struct thread, elem)->priority >
		 thread_get_priority();

  intr_set_level (old_level);

  if (result) thread_yield ();
}

/* Used for list elem comparison */
...
/* Update priority of the thread in the non-mlfqs way */
...
/* Used in THREAD FOREACH, to update RECENT CPU od all threads per second */
void thread_update_recent_cpu (struct thread *t, void *aux UNUSED)
{
  t->recent_cpu = FP_ADD_MIX (
		    FP_DIV(
			FP_MULT(
			  FP_MULT_MIX (load_avg, 2), t->recent_cpu),
...
/* This function handles what will happen per second with mlfqs */
...
/* Updates recent_cpu for all threads based on their load_avg */
...
/* Updates priority for all threads based on their recent_cpu and nice values */
void update_priorities_for_all(void)
{
    struct list_elem *e;
    for (e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e))
    {
