...
struct thread
{
   ...
   struct list_elem sleepelem;		// list for sleeping threads
   int64_t remaining_time_to_wake_up;	// Ticks remaining from waking, init 0

   /* Owned by thread.c and synch.c */
   int real_priority;		// Real priority while being donated
   int nice;			// Determines how nice a thread should be to other threads

   fixed_t recent_cpu;		// The most recent cpu

   struct list locks_held;	// All locks held
   struct lock *current_lock;	// The thread has been locked by this lock
...
/* For thread priority and timing */
